...
function dePalabrasAFrase(palabras) {
  ...
  for (i = 0; i < palabras.length; i++) {
    if (i === 0) string = palabras[i];
    else (string = string + " " + palabras[i]);
  }
  return string;
  // leyendo la solución me di cuenta que era tan simple como:
...
function promedioResultadosTest(resultadosTest) {
  ...
  return promedio = sumaResultados / resultadosTest.length;
}
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  var almaceno = arreglo[0];
  for (i = 1; i < arreglo.length; i++) {
    if (!(arreglo[i] === almaceno)) return false;
  }
  return true;
}
